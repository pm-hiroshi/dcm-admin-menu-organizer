...
### 1.2.5 (2025-12-11)
- **CHANGED**: 「デフォルト状態のメニュー順序を挿入」ボタンに文言を統一し、挿入テキストのヘッダー行を削除
- **CHANGED**: インポート結果は常にURL形式に統一（スラッグのみは `admin.php?page=slug` に変換）

### 1.2.4 (2025-12-11)
- **NEW**: プラグイン一覧にリセットリンクを追加（設定を初期化）
- **CHANGED**: 通知のプラグイン名をヘッダから取得し、ハードコードを排除
...
- 「デフォルト状態のメニュー順序を挿入」ボタンをtextareaの直下に移動
